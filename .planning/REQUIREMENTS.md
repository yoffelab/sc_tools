...
- [x] **PRV-03**: `sct provenance show <file>` — display provenance for a single output
- [x] **PRV-04**: `sct provenance trace <file>` — trace full lineage DAG via input file references
...
- [x] **SCI-02**: Marker validation report — after cell typing, compute top N marker genes per assigned type, generate dotplot, flag types with low canonical marker expression
...
- [x] **MEM-01**: IO Gateway with tiered loading strategy: h5py for metadata/embeddings, backed mode for summaries, full load only for compute
- [x] **MEM-02**: `sct estimate <command> <args>` — pre-execution estimation of peak memory and runtime based on cell/gene count and method
