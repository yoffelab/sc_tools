...
must_haves:
  truths:
    ...
    - "`sct benchmark integration --report` generates HTML benchmark report with subsampling params (D-12 item 3)"
  artifacts:
    ...
    - path: "sc_tools/cli/benchmark.py"
      provides: "sct benchmark integration command with --report flag"
  ...
  key_links:
    ...
    - from: "sc_tools/cli/benchmark.py"
      to: "sc_tools/bm/report.py"
      via: "from sc_tools.bm.report import generate_benchmark_report"
      pattern: "from sc_tools\\.bm\\.report import"
---
...
Output: Working `sct preprocess run` and `sct benchmark integration` commands returning CLIResult JSON. Benchmark --report flag generates HTML with subsampling params per D-12.
</objective>
...
def generate_benchmark_report(
    ...
    *,
    ...
    """Generate HTML benchmark report. benchmark_params dict is rendered in the report header."""
```
...
```
  ...
  <verify>
    <automated>cd /Users/junbumkim/Documents/sc_tools && python -c "from sc_tools.cli.preprocess import preprocess_app, preprocess_run, _detect_modality; print('OK')" && python -c "from typer.testing import CliRunner; from sc_tools.cli import app; r = CliRunner().invoke(app, ['preprocess', '--help']); assert r.exit_code == 0 and 'run' in r.output, f'preprocess --help failed'; print('--help OK')" && python -m pytest sc_tools/tests/test_cli.py -x -q</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `sct preprocess --help` exits 0 and shows "run" subcommand
    - Existing test_cli.py tests still pass
  </acceptance_criteria>
  <done>sct preprocess run dispatches modality-aware recipe with auto-detection (D-08), writes output h5ad, communicates parameters clearly (D-13). Command registered and reachable via --help.</done>
</task>

<task type="auto">
  <name>Task 2: Implement sct benchmark integration command with --report flag</name>
  <files>sc_tools/cli/benchmark.py, sc_tools/bm/report.py</files>
  <read_first>
    - sc_tools/cli/__init__.py (cli_handler, _check_deps)
    - sc_tools/bm/integration.py (compare_integrations signature, embedding_files parameter)
    - sc_tools/bm/report.py (generate_benchmark_report signature, benchmark_params parameter)
...
Additionally, read `sc_tools/bm/report.py` and verify it accepts a `benchmark_params` keyword argument in `generate_benchmark_report()`. If the function does not yet accept `benchmark_params`, add it:

1. Add `benchmark_params: dict | None = None` parameter to the function signature.
2. In the report template rendering, if `benchmark_params` is provided, render a "Benchmark Parameters" section in the HTML header showing: n_cells_original, subsampled (yes/no), subsample_n, seed, batch_key, celltype_key, batch_weight, bio_weight, resolution, methods list.
3. This ensures D-12 item 3 is satisfied: subsampling decisions appear in the benchmark HTML report.
  </action>
  <verify>
    <automated>cd /Users/junbumkim/Documents/sc_tools && python -c "from sc_tools.cli.benchmark import benchmark_app, benchmark_integration; print('OK')" && python -c "from typer.testing import CliRunner; from sc_tools.cli import app; r = CliRunner().invoke(app, ['benchmark', '--help']); assert r.exit_code == 0 and 'integration' in r.output, f'benchmark --help failed'; print('--help OK')" && python -m pytest sc_tools/tests/test_cli.py -x -q</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `sc_tools/cli/benchmark.py` contains `generate_report: bool = typer.Option(False, "--report"` (per D-12)
    - `sc_tools/cli/benchmark.py` contains `from sc_tools.bm.report import generate_benchmark_report` (D-12 item 3)
    - `sc_tools/cli/benchmark.py` contains `benchmark_params=benchmark_params` (passes subsampling to report)
    ...
    - `sc_tools/cli/benchmark.py` contains `embedding_files=embedding_files`
    ...
    - `sc_tools/bm/report.py` accepts `benchmark_params` keyword argument
    - `sct benchmark --help` exits 0 and shows "integration" subcommand
    - Existing test_cli.py tests still pass
  </acceptance_criteria>
  <done>sct benchmark integration loads pre-computed h5ad embeddings from --from-dir, computes ranked metrics, records subsampling in output (D-07, D-12, D-13). --report flag generates HTML benchmark report with subsampling params per D-12 item 3. Command registered and reachable via --help.</done>
</task>
...
runner = CliRunner()
for cmd in ['preprocess', 'benchmark']:
...
- Benchmark --report generates HTML with subsampling params via bm/report.py (D-12 item 3)
- Both commands support --project-dir (D-03) and have sensible defaults (D-06)
- Both commands use _check_deps for fast-fail (CMD-08)
- Both commands reachable via `sct <cmd> --help` (WARNING 2 fix)
</success_criteria>
