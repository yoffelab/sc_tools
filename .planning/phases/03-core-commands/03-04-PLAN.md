...
must_haves:
  truths:
    ...
    - "TestPreprocessRun and TestBenchmarkIntegration classes exist and test CMD-02 and CMD-04"
  artifacts:
    - path: "sc_tools/tests/test_cli_commands.py"
      provides: "CLI command integration tests for all commands"
      contains: "class TestPreprocessRun"
...
Purpose: Test coverage for all Phase 3 commands (TST-05), including CMD-02 (preprocess) and CMD-04 (benchmark). E2E test structure (TST-06), and verification that CLI and MCP share the same CLIResult type (CMD-07).
Output: test_cli_commands.py with CliRunner tests for all commands including preprocess and benchmark, test_cli_e2e.py with skipif scaffold, verified MCP shared backend pattern
</objective>
...
<task type="auto">
  <name>Task 1: Write CLI integration tests for all commands</name>
  ...
  <read_first>
    ...
    - sc_tools/cli/preprocess.py (preprocess_run args and _detect_modality)
    - sc_tools/cli/benchmark.py (benchmark_integration args)
...
class TestPreprocessRun:
    ...
    def test_detect_modality_raises_on_unknown(self, adata_100):
        ...
        # Remove modality hint and use large panel
        adata_100.uns.pop("modality", None)
        with pytest.raises(SCToolsDataError, match="Cannot auto-detect"):
            _detect_modality(adata_100)
...
- TestPreprocessRun tests CMD-02: --help options, missing file, bad integration, _detect_modality unit tests
- TestBenchmarkIntegration tests CMD-04: --help options (including --report), missing dir, empty dir
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - File contains `class TestPreprocessRun` with at least 4 test methods (WARNING 1 fix)
    - File contains `class TestBenchmarkIntegration` with at least 3 test methods (WARNING 1 fix)
    ...
    - TestPreprocessRun tests _detect_modality directly (D-08)
    - TestBenchmarkIntegration checks --report flag appears in --help
  </acceptance_criteria>
  <done>CLI integration tests for all commands: validate, status, qc run, preprocess run (CMD-02), benchmark integration (CMD-04), dep check, and shared Result (CMD-01, CMD-02, CMD-03, CMD-04, CMD-05, CMD-07, CMD-08, TST-05)</done>
</task>
...
<success_criteria>
- test_cli_commands.py covers CMD-01, CMD-02, CMD-03, CMD-04, CMD-05, CMD-07, CMD-08 with fixture-based tests
- TestPreprocessRun class exists with tests for CMD-02 (WARNING 1 fix)
- TestBenchmarkIntegration class exists with tests for CMD-04 (WARNING 1 fix)
