...
| Slug | Old code | Notes |
|------|----------|-------|
| `ingest_raw` | p0a | HPC platform tools |
| `ingest_load` | p0b | Per-sample AnnData loader |
| `qc_filter` | p1 | QC + concatenation |
| `metadata_attach` | p2 | Clinical metadata join |
| `preprocess` | p3 | Normalize + integrate + cluster |
| `demographics` | p3.5 | Cohort demographics (optional branch) |
| `scoring` | p3.5b | Gene scoring + auto cell typing |
| `celltype_manual` | p4 | Manual cell typing (optional, iterative) |
| `biology` | p5 | Downstream biology |
| `meta_analysis` | p6/p7 | Meta analysis (optional) |
...
| `ingest_load` | `data/{sample_id}/adata.p0.h5ad` | Per-sample AnnData loaded from platform output via `sc_tools.ingest.loaders`. |
...
| `qc_filter` | `results/adata.raw.h5ad` | Concatenated, QC-filtered AnnData across all samples. |
...
**Legacy names (migration):** Existing projects may still use `adata.raw.p1.h5ad`, `adata.annotated.p2.h5ad`, `adata.normalized.p3.h5ad`, `adata.normalized.scored.p35.h5ad`, `adata.celltyped.p4.h5ad`. Both old and new names are accepted during transition; `validate_checkpoint()` reports which convention is in use.
...
|------------|---------------------------|
| **adata.h5ad** (per-sample, `ingest_load`) | `obs['sample']`, `obs['library_id']`, `obs['raw_data_dir']`, `obsm['spatial']`; `X` raw counts; single sample only (not concatenated). Set by `sc_tools.ingest.loaders`. **IMC only (optional):** `adata.uns['spatial'][sample_id]` may contain image data when `load_imc_sample(load_images=True)` is used — see IMC image schema below. |
| **adata.raw.h5ad** (`qc_filter`) | `obs['sample']`, `obs['raw_data_dir']` (or equivalent), `obsm['spatial']`; `X` raw counts; no normalization; all samples concatenated. |
...
| `figures/QC/post_integration_qc_YYYYMMDD.html` | Post-integration QC HTML report (Phase 3 exit). Batch score is primary metric; bio metrics are informational only (celltype labels not yet validated). Date-versioned. |
| `figures/QC/post_celltyping_qc_YYYYMMDD.html` | Post-celltyping QC HTML report (Phase 4 exit). Full bio metrics (ARI, NMI, ASW celltype) now meaningful with validated celltypes. Date-versioned. |
...
**Snakemake:** Use `$(PROJECT)` or config-based paths for all project paths, e.g. `$(PROJECT)/results/adata.raw.h5ad`.
...
**Outputs:** `$(PROJECT)/results/adata.raw.h5ad`, `$(PROJECT)/figures/QC/raw/*`. Must satisfy required metadata for `qc_filter` (Section 2.2).
...
| Active checkpoints (p0-p4) | HPC scratch | h5ad | `sftp://brb//athena/.../adata.raw.h5ad` |
...
| `scripts/ingest.py` (per-sample loader) | `ingest_load` | `$(PROJECT)/data/{sample_id}/adata.p0.h5ad` |
| `scripts/ingest.py` (concat + QC) | `qc_filter` | `$(PROJECT)/results/adata.raw.h5ad` |
...
2. **Checkpoint nomenclature:** New pipelines and scripts **must** write AnnData checkpoints using the standard names in Section 2.1 (`adata.raw.h5ad`, `adata.annotated.h5ad`, `adata.normalized.h5ad`, `adata.scored.h5ad`, `adata.celltyped.h5ad`). Legacy p-code names (`adata.raw.p1.h5ad`, etc.) are accepted during transition but deprecated. Validators should check required metadata (Section 2.2) when loading checkpoints.
