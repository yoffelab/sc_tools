...
1. Project `Mission.md` — todo list and current priority (lives in `projects/<platform>/<project>/Mission.md`, symlinked into wiki)
2. `docs/wiki/projects/{name}/hypotheses.md` — scientific questions (wiki-native)
...
| `sc_tools/Mission.md` | Symlink → root `Mission.md` | Yes |
| `sc_tools/Architecture.md` | Symlink → root `Architecture.md` | Yes |
| `sc_tools/skills.md` | Symlink → root `skills.md` | Yes |
...
All QC reports are date-versioned (`YYYYMMDD`) and saved to `figures/QC/`. Full validation contracts: Architecture.md Section 2.2. For project status and knowledge base, see `docs/wiki/`.
