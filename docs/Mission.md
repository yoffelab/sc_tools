...
Pipeline phases and checkpoint specs: see [[Architecture]] §2.
...
- [x] **Snakemake `ingest_raw` rules:** `spaceranger_count` and `phase0` target in robin, ggo_visium, and create_project.sh template. Driven by `metadata/phase0/all_samples.tsv`.
...
- [x] **`ingest_load` checkpoint script (`scripts/ingest.py`):** Generic CLI reads batch manifest, dispatches to modality loaders (`_get_loader()`), saves per-sample `data/{sample_id}/adata.ingested.h5ad` (`--save-per-sample`), concatenates, saves final output. 5 tests in `test_ingest_script.py`.
- [ ] **`ingest_load` Snakemake rule:** `adata_ingested` rule with sentinel `data/{sample_id}/.adata.ingested.done`. Wire `scripts/ingest.py` into project Snakefiles.
...
**Input:** Per-sample `data/{sample_id}/adata.ingested.h5ad` produced in `ingest_load`.

- [x] **Load `ingest_load` checkpoints:** Read all per-sample `adata.ingested.h5ad` files listed in `metadata/phase0/all_samples.tsv`. Implemented in `scripts/ingest.py`.
...
- [x] **`generate_pre_filter_report()`**: `qc_filter` entry. Output: `figures/QC/pre_filter_qc_YYYYMMDD.html`.
- [x] **`generate_post_filter_report()`**: `metadata_attach` exit. Pre-vs-post comparison, HVG/SVG. Output: `figures/QC/post_filter_qc_YYYYMMDD.html`.
- [x] **`generate_post_integration_report()`**: `preprocess` exit. UMAP grid, cluster distribution, integration benchmark. **Batch score is the primary metric** (celltype labels are preliminary). Output: `figures/QC/post_integration_qc_YYYYMMDD.html`. Accepts `comparison_df` to skip recomputation.
- [x] **`generate_post_celltyping_report()`**: `celltype_manual` exit. Full bio + batch metrics with validated celltypes. Re-scores `results/tmp/integration_test/{method}.h5ad` if available. Output: `figures/QC/post_celltyping_qc_YYYYMMDD.html`. 3 tests.
...
- [ ] Preprocessed projects may skip to `preprocess` or `celltype_manual` if already annotated.
...
- [ ] **resolVI integration (`sc_tools.pp.integrate`):** Add `run_resolvi()` — spatial-aware integration using resolVI (scvi-tools). Leverages spatial coordinates as priors; applicable to Visium, Visium HD, Xenium, CosMx. Soft dep (`pip install scvi-tools[resolvi]`). Add to integration benchmark candidate list for transcriptomic modalities.
...
- [x] **4 QC HTML reports:** Pre-filter (`qc_filter`), post-filter (`metadata_attach`), post-integration (`preprocess`, batch-focused), **post-celltyping** (`celltype_manual`, full bio metrics). All 4 implemented. See Architecture.md Section 2.5.
...
- [ ] **No automated cell typing here** (moved to `scoring`).
...
- [ ] `scoring` is a separate branch from `preprocess` (parallel to `demographics`); connects to `celltype_manual`. `celltype_manual` is skippable if automated cell typing is adequate.

---

### celltype_manual: Manual Cell Typing (Human-in-Loop, Iterative; Skippable)
...
- [ ] **`meta_analysis` (ROI aggregation):** Aggregate to ROI level and patient level (mean expression, celltype counts).
- [ ] **`meta_analysis` (downstream):** Downstream analysis on aggregated ROI/patient data.
...
- **Entry points:** See Phasing scheme (Section 1). Start at `preprocess` (preprocessed), `scoring` (clustered), or `celltype_manual` (phenotyped) as appropriate.
...
1. **Project unit tests (ggo_visium)** — Establish baseline; validate Makefile and scripts (`qc_filter` through `biology`) run with fixtures.
...
- **Deconvolution spatial plots:** `scripts/plot_deconvolution_spatial.py` saves per-library PNGs at 300 DPI alongside PDF. Method-specific output dirs (`figures/deconvolution/{method}/`). Spot sizing `120000/n_spots`; 98th percentile vmax; white background. Both Snakefiles updated (`scoring` rules).
...
- [ ] Wire end-to-end IMC Snakemake workflow (`ingest_raw` → `ingest_load`) for lymph_dlbcl and ggo-imc.
...
**4. `ingest_load` checkpoint script — PARTIALLY DONE**
...
| 8 | **BioData OOP Hierarchy v1.0** | Done | JTI hierarchy (5 subtypes), 95 platforms in PlatformSpec, Subject/Sample tables, 7 MCP tools, Alembic migrations `0003`-`0005`, `seed_biodata.py`. 701 tests pass. |
| 9 | **BioData OOP Hierarchy v1.1** | Done | 19 modalities across 95 platforms, `modality` column (migration `0006`), auto-fill (modality + defaults + format), dual-write in `register_dataset()`, `list_modalities` MCP tool, 3 wiki pages, `seed_clinical_data.py` (775 real patients). 752 tests pass. |
