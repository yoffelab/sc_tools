#!/bin/bash
#SBATCH --job-name=m_reports
#SBATCH --partition=scu-cpu
#SBATCH --mem=64G
#SBATCH --cpus-per-task=8
#SBATCH --time=02:00:00
#SBATCH --output=/home/fs01/juk4007/elementolab/projects/ibd_spatial/logs/generate_reports.log
...
python -c "import sc_tools" 2>/dev/null || {
    ...
    pip install -e "${SC_TOOLS}" --no-deps 2>/dev/null || {
        echo "sc_tools not found at ${SC_TOOLS}, trying pip install"
        pip install jinja2 plotly 2>/dev/null
    }
}

# Ensure jinja2 and plotly are available
pip install jinja2 plotly 2>/dev/null || true

mkdir -p ${WORKDIR}/figures/QC

python ${WORKDIR}/scripts/generate_reports.py --milestone all
