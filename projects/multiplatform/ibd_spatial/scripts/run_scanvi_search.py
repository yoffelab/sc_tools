#!/usr/bin/env python3
"""scANVI hyperparameter search: parallelized via SLURM job arrays.
...
Design:
- 64 configs total (fits in 16 batches of 4 on g0001 with 4x A100)
- Fixed (confirmed optimal): nb, gene-batch, celltype_broad, batch_size=512
- Search: n_layers x n_latent x n_hidden x classification_ratio x pretrain x finetune
- Grid: 3 x 4 x 3 x 4 x 2 x 2 = 576 full; Latin hypercube sampled to 64

Reads:  results/m2_benchmark/adata.m2.h5ad (164K cells, 2552 genes)
Writes: results/scanvi_search/config_{idx}.csv
"""
...
OUTDIR = WORKDIR / "results" / "scanvi_search"
...
PARAM_GRID = {
    "n_layers": [2, 3, 4],
    "n_latent": [20, 30, 40, 50],
    "n_hidden": [128, 256, 512],
    "classification_ratio": [75, 100, 125, 150],
    "pretrain_epochs": [100, 150],
    "finetune_epochs": [50, 75],
}
...
def build_config_list(n_configs=64, seed=42):
    """Build a Latin hypercube sample of configs from the full grid.

    Strategy:
    1. Always include the known-best A6 config (n_layers=3, n_latent=30,
       n_hidden=256, cls_ratio=100, pretrain=100, finetune=50)
    2. Always include neighbors of A6 (one param changed at a time) = ~12 configs
    3. Fill remaining slots with LHS to cover the space uniformly
    """
    import itertools
    ...
    # A6 reference point (the known best)
    a6 = (3, 30, 256, 100, 100, 50)

    # Must-include configs: A6 + one-param neighbors
    must_include = {a6}

    # Neighbors: change one param at a time from A6
    for dim, param_values in enumerate(values):
        for val in param_values:
            neighbor = list(a6)
            neighbor[dim] = val
            must_include.add(tuple(neighbor))

    # Convert to list, ensure all are in full_grid
    must_include = [c for c in must_include if c in full_grid]
    print(f"Must-include configs: {len(must_include)}")

    # Remaining slots via stratified random sampling
    remaining_pool = [c for c in full_grid if c not in must_include]
    rng = np.random.RandomState(seed)
    n_remaining = n_configs - len(must_include)

    if n_remaining > 0:
        # Stratified: ensure each param value appears roughly equally
        selected_indices = rng.choice(len(remaining_pool), size=min(n_remaining, len(remaining_pool)), replace=False)
        extra = [remaining_pool[i] for i in selected_indices]
    else:
        extra = []

    configs = must_include + extra
    ...
    for i, vals in enumerate(configs):
        ...
        cfg["config_idx"] = i
        cfg["name"] = (
            f"S{i:02d}_L{cfg['n_layers']}_d{cfg['n_latent']}_h{cfg['n_hidden']}"
            f"_c{cfg['classification_ratio']}_p{cfg['pretrain_epochs']}_f{cfg['finetune_epochs']}"
        )
...
ALL_CONFIGS = build_config_list(n_configs=64, seed=42)
N_CONFIGS = len(ALL_CONFIGS)


# ============================================================
# Metric helpers (from run_scanvi_max.py)
# ============================================================
...
def run_one_config(config_idx: int):
    ...
    # Load data (prefer /tmp copy if SLURM script cached it there)
    local_path = Path("/tmp/adata.m2.h5ad")
    load_path = local_path if local_path.exists() else INPUT_PATH
...
if __name__ == "__main__":
    ...
    if "SLURM_ARRAY_TASK_ID" in os.environ:
        ...
        for cfg in ALL_CONFIGS:
            print(f"  [{cfg['config_idx']:2d}] {cfg['name']}")
        sys.exit(1)

    if config_idx < 0 or config_idx >= N_CONFIGS:
        print(f"ERROR: config_idx={config_idx} out of range [0, {N_CONFIGS})")
        sys.exit(1)
    ...
    print(f"Config index: {config_idx}/{N_CONFIGS}")
    print()
