...
def get_cache_key(sc_data_file: str, celltype_key: str, sc_batch_key: str, 
                 n_genes_max: int, skip_hvg: Optional[bool] = None) -> str:
    """
    ...
    key_string = f"{sc_data_file}:{mtime}:{celltype_key}:{sc_batch_key}:{n_genes_max}:{skip_hvg_str}"
    key_hash = hashlib.md5(key_string.encode()).hexdigest()
    return key_hash


def load_cached_signatures(cache_path: Path, 
                          logger_instance: Optional[logging.Logger] = None) -> Optional[List[str]]:
    """
        ...
        if cache_path.exists():
            with open(cache_path, 'rb') as f:
                ...
                if isinstance(cache_data, dict) and 'signature_genes' in cache_data:
                    log.info(f"   ✅ Loaded {len(cache_data['signature_genes'])} signature genes from cache")
                    return cache_data['signature_genes']
                elif isinstance(cache_data, list):
                    # Backward compatibility: old format was just a list
                    log.info(f"   ✅ Loaded {len(cache_data)} signature genes from cache (old format)")
                    return cache_data
