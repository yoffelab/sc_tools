...
def add_significance_bars(ax: plt.Axes,
    ...
    sig_comparisons = [c for c in comparisons 
                      if c.get('adj_pval', c.get('pval', 1.0)) < 0.05]
    ...
    for idx, comp in enumerate(sig_comparisons):
        ...
        ax.text(x_asterisk, y_bar + bar_height * 0.4, asterisk,
...
def plot_boxplot_with_stats(adata,
                            value_col: str,
                            group_col: str,
                            ...
                            show_comparisons: bool = True) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    # Prepare data
    groups = adata.obs[group_col].cat.categories if hasattr(adata.obs[group_col], 'cat') else adata.obs[group_col].unique()
    plot_data = []
