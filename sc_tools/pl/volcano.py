...
from ..tl.testing import mwu, fdr_correction


def volcano_plot(adata: ad.AnnData,
                 group_col: str,
                 value_col: str,
                 group1: str,
                 group2: str,
                 ...
                 max_annotations: Optional[int] = None) -> Tuple[plt.Figure, plt.Axes, pd.DataFrame]:
    """
...
def volcano_plot_faceted(adata: ad.AnnData,
                        group_col: str,
                        value_cols: List[str],
                        ...
                        pval_threshold: float = 0.05,
                        annotate_significant: bool = True,
                        signatures_include: Optional[List[str]] = None,
                        signatures_exclude: Optional[List[str]] = None) -> Tuple[plt.Figure, pd.DataFrame]:
    """
    ...
    if signatures_include is not None or signatures_exclude is not None:
        ...
        value_cols = filter_signatures(value_cols, include=signatures_include, exclude=signatures_exclude)
    ...
    for group1, group2, comp_name in comparisons:
        ...
        plot_df['color'] = 'gray'
        plot_df.loc[(plot_df['adj_pval'] < pval_threshold) & (plot_df['log2FC'] > fc_threshold), 'color'] = 'red'
        plot_df.loc[(plot_df['adj_pval'] < pval_threshold) & (plot_df['log2FC'] < -fc_threshold), 'color'] = 'blue'
        plot_df.loc[(plot_df['adj_pval'] < pval_threshold) & (plot_df['log2FC'].abs() <= fc_threshold), 'color'] = 'orange'
    ...
    for ax_idx, ((group1, group2, comp_name), plot_df) in enumerate(zip(comparisons, all_plot_data)):
        ax = axes[ax_idx]
