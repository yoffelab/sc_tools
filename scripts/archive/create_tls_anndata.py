...
def create_tls_anndata(
    ...
    if not has_solidity:
        warnings.warn(
            f"Column '{solidity_type_col}' not found in adata.obs. Solidity information will not be included."
        )
        ...
        if has_solidity:
            ...
            if len(candidates) > 1:
                ...
                candidates_ordered = sorted(
                    candidates,
                    key=lambda x: (
                        solidity_tie_order.index(x)
                        if x in solidity_tie_order
                        else len(solidity_tie_order)
                    ),
                )
    ...
    # Create new anndata
    adata_tls_out = ad.AnnData(X=X_mean_array, obs=obs_df, var=var_df, dtype=X_mean_array.dtype)
