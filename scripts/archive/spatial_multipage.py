...
DEPRECATED: The canonical implementation lives under ggo_visium and uses sc_tools:
  projects/visium/ggo_visium/scripts/spatial_multipage_colocalization.py
That script reads scores from obsm, outputs to figures/manuscript/macrophage_localization/
and figures/manuscript/neutrophil_cytotoxic_tcell_localization/, and uses
sc_tools.pl.spatial and sc_tools.tl.colocalization. Use that script for new work.
...
def plot_spatial_annotation(adata_sub, library_id, ax):
    ...
    if "pathologist_annotation" not in adata_sub.obs.columns:
        ax.text(
            0.5,
            0.5,
            "pathologist_annotation not found",
            ha="center",
            va="center",
            transform=ax.transAxes,
        )
...
def plot_spatial_solidity(adata_sub, library_id, ax):
    ...
    if "solidity" not in adata_sub.obs.columns:
        ax.text(0.5, 0.5, "solidity not found", ha="center", va="center", transform=ax.transAxes)
...
def plot_spatial_score(
    adata_sub, library_id, score_col, ax, title, cmap="coolwarm", vmin=None, vmax=None
):
    """Plot spatial score."""
    if score_col not in adata_sub.obs.columns:
        ax.text(
            0.5, 0.5, f"{score_col} not found", ha="center", va="center", transform=ax.transAxes
        )
...
def create_multipage_pdf(
    adata, output_pdf, proliferation_sig=PROLIFERATION_SIG, macrophage_sig=MACROPHAGE_SIG
):
    """
...
def plot_proportion_boxplot(adata, output_dir):
    ...
    ax.set_title(
        "Proportion of Non-Zero Truncated Similarity\nby Solidity Condition",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
    ...
    for solidity in solidity_order:
        solidity_groups[solidity] = prop_df[prop_df["solidity"] == solidity][
            "proportion_nonzero"
        ].values
    ...
    if len(p_values) > 0:
        ...
        ax.set_ylim(
            bottom=prop_df["proportion_nonzero"].min() - 0.05 * y_range, top=bar_y + 2 * bar_height
        )
...
def main():
    ...
    if PROLIFERATION_SIG not in adata.obs.columns:
        ...
        print("   Available signature columns (first 10):")
    ...
    if MACROPHAGE_SIG not in adata.obs.columns:
        ...
        print("   Available macrophage signatures:")
        mac_cols = [
            col for col in adata.obs.columns if "macrophage" in col.lower() or "mac" in col.lower()
        ]
    ...
    if PROLIFERATION_SIG in adata.obs.columns and MACROPHAGE_SIG in adata.obs.columns:
        adata.obs["truncated_similarity"] = calculate_truncated_similarity(
            adata.obs[PROLIFERATION_SIG], adata.obs[MACROPHAGE_SIG]
        )
    else:
        print(
            f"Warning: Missing signature columns. Proliferation: {PROLIFERATION_SIG in adata.obs.columns}, "
