...
def check_memory_threshold(threshold_mb: float = 8000, threshold_percent: float = 85.0) -> bool:
    ...
    if mem['rss_mb'] > threshold_mb:
        logger.warning(f"[MEMORY WARNING] RSS ({mem['rss_mb']:.1f}MB) exceeds threshold ({threshold_mb}MB)")
        return False
    
    if mem['system_percent'] > threshold_percent:
        logger.warning(f"[MEMORY WARNING] System memory ({mem['system_percent']:.1f}%) exceeds threshold ({threshold_percent}%)")
        return False
...
def load_single_cell_data(sc_data_file: str, celltype_key: str, 
                          sc_batch_key: str, qc_labels: List[str]) -> Tuple[ad.AnnData, List[str]]:
...
def select_signature_genes(sc_adata: ad.AnnData, celltype_key: str, 
                           ...
                           sc_data_file: Optional[str] = None) -> List[str]:
...
def get_library_ids(spatial_data_file: str, spatial_batch_key: str) -> List[str]:
    ...
    if spatial_batch_key not in spatial_metadata.columns:
        if 'sample_id' in spatial_metadata.columns:
            ...
            print(f"   Using 'sample_id' instead of 'library_id'")
        else:
            raise ValueError(f"Neither 'library_id' nor 'sample_id' found in spatial data")
...
def deconvolve_tangram(sc_adata: ad.AnnData, spatial_adata_lib: ad.AnnData,
                       shared_genes: List[str], celltype_key: str,
                       output_path: str, num_epochs: int) -> Optional[np.ndarray]:
    ...
    try:
        ...
        if os.path.exists(output_path):
            ...
            ad_map = tg.map_cells_to_space(
                sc_copy, spatial_lib_sig,
                mode='clusters', cluster_label=celltype_key,
...
def deconvolve_cell2location(sc_adata: ad.AnnData, spatial_adata_lib: ad.AnnData,
                             shared_genes: List[str], celltype_key: str,
                             output_path: str, max_epochs: int) -> Optional[np.ndarray]:
    ...
    try:
        ...
        if not check_memory_threshold(threshold_mb=10000, threshold_percent=85.0):
            ...
            num_samples = 500
            batch_size = 1500
            logger.info(f"Cell2location: Reducing parameters (epochs={actual_epochs}, samples={num_samples}) due to memory")
        ...
        c2l.models.Cell2location.setup_anndata(
            spatial_lib_sig, batch_key=None
        )
...
def deconvolve_destvi(sc_adata: ad.AnnData, spatial_adata_lib: ad.AnnData,
                      shared_genes: List[str], celltype_key: str,
                      output_path: str, max_epochs: int) -> Optional[np.ndarray]:
...
def process_library(lib_id: str, sc_adata: ad.AnnData, spatial_data_file: str,
                    spatial_batch_key: str, candidate_genes: List[str],
                    celltype_key: str, unique_cts: List[str],
                    methods: List[str], config: Dict) -> Tuple[Optional[np.ndarray], List[str]]:
    ...
    try:
        ...
        if not check_memory_threshold():
            logger.warning(f"[MEMORY] Memory usage high before loading {lib_id}, performing cleanup...")
            aggressive_cleanup()
        ...
        # Find shared genes
        shared_genes = list(set(candidate_genes).intersection(
            sc_adata.var_names, spatial_adata_lib.var_names
        ))[:config['n_genes_max']]
        ...
        if not check_memory_threshold():
            logger.warning(f"[MEMORY] Memory usage high before processing {lib_id}, reducing gene set...")
        ...
        for method in methods:
            ...
            if not check_memory_threshold(threshold_mb=10000, threshold_percent=90.0):
                logger.warning(f"[MEMORY] Memory usage too high for {method}, skipping to next method...")
                aggressive_cleanup()
                continue
            ...
            if method == 'tangram':
                proportions = deconvolve_tangram(
                    sc_adata, spatial_adata_lib, shared_genes, celltype_key,
                    output_path, config['tangram_epochs']
                )
            elif method == 'cell2location':
                proportions = deconvolve_cell2location(
                    sc_adata, spatial_adata_lib, shared_genes, celltype_key,
                    output_path, config['cell2location_max_epochs']
                )
            elif method == 'destvi':
                proportions = deconvolve_destvi(
                    sc_adata, spatial_adata_lib, shared_genes, celltype_key,
                    output_path, config['destvi_max_epochs']
                )
            else:
            ...
            if proportions is not None:
                ...
                if proportions.shape[0] == spatial_adata_lib.n_obs:
                    ...
                    if proportions.shape[1] != len(unique_cts):
                        print(f"   Warning: Proportions shape {proportions.shape} doesn't match expected ({spatial_adata_lib.n_obs}, {len(unique_cts)})")
                        ...
                        if proportions.shape[1] > 0:
                            ...
                            if proportions.shape[1] < len(unique_cts):
                                # Pad with zeros
                                padding = np.zeros((proportions.shape[0], len(unique_cts) - proportions.shape[1]))
                    ...
                    logger.warning(f"Proportions shape mismatch ({proportions.shape[0]} spots vs {spatial_adata_lib.n_obs}), trying next method...")
                    proportions = None
                    aggressive_cleanup()
...
def integrate_proportions(all_proportions: List[np.ndarray], all_spot_indices: List[str],
                          spatial_data_file: str, unique_cts: List[str],
...
def main():
    ...
    sc_adata, unique_cts = load_single_cell_data(
        CONFIG['sc_data_file'], CONFIG['celltype_key'],
        CONFIG['sc_batch_key'], qc_labels
    )
    
    # Select signature genes
    candidate_genes = select_signature_genes(
        sc_adata, CONFIG['celltype_key'],
        CONFIG['sc_batch_key'], CONFIG['n_genes_max'],
    ...
    all_spot_indices = []
    processed_libraries = []
    method_usage = {method: 0 for method in CONFIG['methods']}
    
    for lib_id in tqdm(library_ids, desc="Processing libraries"):
        proportions, successful_methods = process_library(
            lib_id, sc_adata, CONFIG['spatial_data_file'],
            spatial_batch_key, candidate_genes,
            CONFIG['celltype_key'], unique_cts,
    ...
    integrate_proportions(
        all_proportions, all_spot_indices,
        CONFIG['spatial_data_file'], unique_cts,
        CONFIG['celltype_key']
    )
    ...
    if len(processed_libraries) > 0:
        logger.info(f"Output file: results/adata.deconvolution.h5ad")
        logger.info(f"Cell type proportions stored in: adata.obsm['cell_type_proportions']")
