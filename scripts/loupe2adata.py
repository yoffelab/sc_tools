...
if not os.path.exists(raw_filename):
    ...
    for path, loc_path in tqdm(zip(loupe_files, location_files)):
        a_ = sq.read.visium(
            path, counts_file="filtered_feature_bc_matrix.h5"
        )  # load_images = False,
...
for path in annotation_files:
    ...
    df.rename(
        columns={"Barcode": "barcode", "Pathologist Annotations": "pathologist_annotation"},
        inplace=True,
    )
...
adata.obs[["solidity_type", "architecture_type"]] = pd.DataFrame(
    solidity_arch.tolist(), index=adata.obs.index
)
