...
def plot_macrophage_colocalization(
    adata,
    macrophage_sig,
    proliferative_sig="sig:Tumor_Cells/Proliferative_Tumor_z",
    group_col="tumor_type",
    output_dir="figures/manuscript/macrophage_localization",
):
    """
    ...
    macrophage_label = (
        macrophage_sig.replace("sig:Liron/", "")
        .replace("_z", "")
        .replace("MoMac", "MoMac ")
        .replace("MoMAc", "MoMac ")
    )

    ax.set_xlabel("Proliferative Score (z-scored)", fontsize=12)
    ax.set_ylabel(f"{macrophage_label} Score (z-scored)", fontsize=12)
    ax.set_title("Macrophage Colocalization with Proliferation", fontsize=13, fontweight="bold")
...
def main():
    ...
    for macrophage_sig in available_mac:
        ...
        correlations = plot_macrophage_colocalization(
            adata, macrophage_sig, PROLIFERATIVE_SIG, group_col="tumor_type"
        )
        if correlations:
    ...
    if all_correlations:
        ...
        corr_matrix = summary_df.pivot(
            index="Macrophage_Signature", columns="Tumor_Type", values="Pearson_r"
        )
        ...
        pval_matrix = summary_df.pivot(
            index="Macrophage_Signature", columns="Tumor_Type", values="p_value"
        )
        ...
        ax.set_title(
            "Macrophage-Proliferation Correlations", fontsize=14, fontweight="bold", pad=15
        )
        ...
        # Set title
        g.fig.suptitle(
            "Macrophage-Proliferation Correlations (Clustered)",
            fontsize=14,
            fontweight="bold",
            y=0.98,
        )
        ...
        clustermap_pdf = Path(
            "figures/manuscript/macrophage_localization/correlation_clustermap.pdf"
        )
        clustermap_png = Path(
            "figures/manuscript/macrophage_localization/correlation_clustermap.png"
        )
    ...
    print("   - Scatterplots saved to: figures/manuscript/macrophage_localization/")
    if all_correlations:
        print("   - Correlations CSV: figures/stats/macrophage_proliferation_correlations.csv")
        print(
            "   - Correlation heatmap: figures/manuscript/macrophage_localization/correlation_heatmap.pdf"
        )
        print(
            "   - Correlation clustermap: figures/manuscript/macrophage_localization/correlation_clustermap.pdf"
        )
    print("=" * 70)
