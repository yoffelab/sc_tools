...
for lib in tqdm(adata.obs["library_id"].unique()):
    ...
    sc.pl.spatial(a, color = spatial_signatures['Tumor_Cells']['EMT_Tumor'], frameon = False, library_id = lib, cmap = 'coolwarm', show = False)
...
import scanpy as sc
from tqdm import tqdm
adata.obs['Tumor Score'] = adata.obs[['sig:Tumor_Cells/Alveolar_Type2_Tumor_z',
    'sig:Tumor_Cells/Alveolar_Type1_Tumor_z',
    'sig:Tumor_Cells/Proliferative_Tumor_z']].mean(axis = 1)
...
for i, ax in tqdm(enumerate(axes.flatten())):
    ...
    sns.kdeplot(a_tmp.obs,
    x = 'Tumor Score', y = 'EMT Score', fill = True, ax = ax, cmap = 'YlOrRd')
    ...
    sns.despine()
plt.tight_layout()
plt.savefig(f'figures/manuscript/EMT_architecture_score_kdeplot.png', bbox_inches = 'tight')
plt.savefig(f'figures/manuscript/EMT_architecture_score_kdeplot.pdf', bbox_inches = 'tight')
plt.close()
