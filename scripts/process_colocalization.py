...
VOLCANO_SIGNATURES_EXCLUDE = ['sig:Immune_Myeloid/Classical_Monocyte', 'sig:Immune_Myeloid/NonClassical_Monocyte', 'sig:Immune_Myeloid/Alveolar_Macrophage', 'sig:Immune_Myeloid/MonoDerived_Macrophage', 'sig:Immune_Myeloid/M1_Macrophage', 'sig:Immune_Myeloid/M2_Macrophage']  # List of signatures to exclude (None = none)
...
def analyze_pearson_correlation(adata, sig_columns, output_dir):
    ...
    ax.set_title('Process Colocalization: Pearson Correlation Matrix', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('figures/process_colocalization/correlation_heatmap.pdf', bbox_inches='tight', dpi=300)
    plt.savefig('figures/process_colocalization/correlation_heatmap.png', bbox_inches='tight', dpi=300)
    plt.close()
    ...
    # Set title
    g.fig.suptitle('Process Colocalization: Pearson Correlation (Hierarchically Clustered)', 
    ...
    plt.savefig('figures/process_colocalization/correlation_clustermap.pdf', bbox_inches='tight', dpi=300)
    plt.savefig('figures/process_colocalization/correlation_clustermap.png', bbox_inches='tight', dpi=300)
    plt.close()
...
def analyze_morans_i(adata, sig_columns, output_dir):
    ...
    if not moran_I_matrix.isna().all().all():
        ...
        plt.tight_layout()
        plt.savefig('figures/process_colocalization/morans_i_heatmap.pdf', bbox_inches='tight', dpi=300)
        plt.savefig('figures/process_colocalization/morans_i_heatmap.png', bbox_inches='tight', dpi=300)
        plt.close()
...
def analyze_neighborhood_enrichment(adata, sig_columns, output_dir):
    ...
    if not nhood_df['nhood_enrichment'].isna().all():
        ...
        plt.tight_layout()
        plt.savefig('figures/process_colocalization/nhood_enrichment_heatmap.pdf', bbox_inches='tight', dpi=300)
        plt.savefig('figures/process_colocalization/nhood_enrichment_heatmap.png', bbox_inches='tight', dpi=300)
        plt.close()
...
def analyze_volcano_plots(adata, sig_columns, output_dir, signatures_include=None, signatures_exclude=None):
    """
    ...
    adata_sub = adata[adata.obs['tumor_type'].isin(['Normal Alveolar Cells', 'Non-Solid Tumor', 'Solid Tumor'])].copy()
    ...
    if signatures_include is not None or signatures_exclude is not None:
        valid_sigs = filter_signatures(valid_sigs, include=signatures_include, exclude=signatures_exclude)
    ...
    for group1, group2, comp_name, comp_title in comparisons:
        ...
        plot_df.loc[(plot_df['adj_pval'] < 0.05) & (plot_df['log2FC'].abs() <= 0.5), 'color'] = 'orange'
    ...
    for ax_idx, (comp_name, comp_title, group1, group2) in enumerate([(c[2], c[3], c[0], c[1]) for c in comparisons]):
        ax = axes[ax_idx]
        ...
        for color in ['gray', 'orange', 'red', 'blue']:
            ...
            if mask.sum() > 0:
                ax.scatter(
                    ...
                    }[color] if ax_idx == 0 else None  # Only show legend on first subplot
                )
        ...
        if annotate_mask.sum() > 0:
            ...
            annotate_df = annotate_df.sort_values('neg_log10_pval', ascending=False).reset_index(drop=True)
            ...
            for ann in all_annotations:
                ax.annotate(
                    ...
                    fontsize=7,
                    alpha=0.8,
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='gray', alpha=0.7, linewidth=0.5),
    ...
    plt.tight_layout()
    plt.savefig(f'figures/process_colocalization/volcano_faceted.pdf', bbox_inches='tight', dpi=300)
    plt.savefig(f'figures/process_colocalization/volcano_faceted.png', bbox_inches='tight', dpi=300)
    plt.close()
    print(f"   ✅ Faceted volcano plot saved: volcano_faceted.pdf")
...
def main():
    ...
    if ANALYSIS_CONFIG['neighborhood_enrichment']:
        results['nhood_enrichment'] = analyze_neighborhood_enrichment(adata, sig_columns, output_dir)
    ...
    if ANALYSIS_CONFIG['pearson_correlation']:
        ...
        print(f"   ✓ Correlation heatmap: figures/process_colocalization/correlation_heatmap.pdf")
        print(f"   ✓ Correlation clustermap: figures/process_colocalization/correlation_clustermap.pdf")
        print(f"   ✓ Correlation clustermap: figures/process_colocalization/correlation_clustermap.pdf")
    
    if ANALYSIS_CONFIG['morans_i']:
        print(f"   ✓ Moran's I results: {output_dir}/morans_i_results.csv")
        print(f"   ✓ Moran's I heatmap: figures/process_colocalization/morans_i_heatmap.pdf")
    
    if ANALYSIS_CONFIG['neighborhood_enrichment']:
        print(f"   ✓ Neighborhood enrichment: {output_dir}/neighborhood_enrichment.csv")
        print(f"   ✓ Enrichment heatmap: figures/process_colocalization/nhood_enrichment_heatmap.pdf")
    
    if ANALYSIS_CONFIG['volcano_plots']:
        print(f"   ✓ Volcano plot results: {output_dir}/volcano_plot_results.csv")
        print(f"   ✓ Faceted volcano plot: figures/process_colocalization/volcano_faceted.pdf")
