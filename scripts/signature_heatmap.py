...
def prepare_tumor_type(adata):
    ...
    if TUMOR_TYPE_COL not in adata.obs.columns:
        ...
        if "pathologist_annotation" not in adata.obs.columns:
            raise KeyError(
                f"Neither '{TUMOR_TYPE_COL}' nor 'pathologist_annotation' found in adata.obs.columns"
            )
...
def create_annotation_dataframe(adata_sub):
    ...
    annotations["Solidity_short"] = (
        annotations["Solidity"].str.replace(" Alveolar Cells", "").str.replace(" Tumor", "")
    )
...
def create_signature_heatmap(
    ...
    sig_df = pd.DataFrame(
        ...
        index=adata_sub.obs_names,
        columns=[
            s.replace(SIGNATURE_PREFIX, "").replace(SIGNATURE_SUFFIX, "") for s in sig_columns
        ],
    )
    ...
    if "Patient" not in annotations.columns or "Solidity" not in annotations.columns:
        raise KeyError(
            f"Annotations dataframe missing required columns. Found: {annotations.columns.tolist()}"
        )
    ...
    patient_color_list = [
        patient_colors.get(p, default_gray) for p in annotations_clustered["Patient"]
    ]
    solidity_color_list = [
        solidity_colors.get(s, default_gray) for s in annotations_clustered["Solidity"]
    ]
    ...
    annotation_colors = pd.DataFrame(
        {"Patient": patient_color_list, "Solidity": solidity_color_list},
    ...
    if cluster:
        ...
        ax_ann_patient = g.fig.add_axes(
            [heatmap_pos.x0, heatmap_pos.y1, heatmap_pos.width, ann_height]
        )
        # patient_colors_array was created earlier with shape (1, n_spots, 3)
        ax_ann_patient.imshow(
            patient_colors_array, aspect="auto", interpolation="nearest", rasterized=True
        )
        ...
        ax_ann_solidity = g.fig.add_axes(
            [heatmap_pos.x0, heatmap_pos.y1 + ann_height, heatmap_pos.width, ann_height]
        )
        # solidity_colors_array was created earlier with shape (1, n_spots, 3)
        ax_ann_solidity.imshow(
            solidity_colors_array, aspect="auto", interpolation="nearest", rasterized=True
        )
        ...
        plt.savefig(
            OUTPUT_DIR / f"signature_clustermap_{output_suffix}.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            OUTPUT_DIR / f"signature_clustermap_{output_suffix}.png", bbox_inches="tight", dpi=300
        )
        plt.close()
        ...
        # Patient annotation
        ax_ann_patient = fig.add_axes(
            [ax.get_position().x0, ax.get_position().y1, ax.get_position().width, 0.02]
        )
        ...
        # Solidity annotation
        ax_ann_solidity = fig.add_axes(
            [ax.get_position().x0, ax.get_position().y1 + 0.02, ax.get_position().width, 0.02]
        )
        ...
        ax.set_title(
            f"Signature Scores Heatmap (sorted by {sort_by[0]} then {sort_by[1]})",
            fontsize=14,
            fontweight="bold",
            pad=20,
        )
        ...
        plt.savefig(
            OUTPUT_DIR / f"signature_heatmap_{output_suffix}.pdf", bbox_inches="tight", dpi=300
        )
        plt.savefig(
            OUTPUT_DIR / f"signature_heatmap_{output_suffix}.png", bbox_inches="tight", dpi=300
        )
        plt.close()
