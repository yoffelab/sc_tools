...
if "moranI" in tumor_adata.uns:
    ...
    for lib_id in tumor_adata.obs["library_id"].unique()[:3]:  # Limit to first 3 libraries
        ...
        for gene in top_svgs[:5]:  # Top 5 genes
            if gene in adata_lib.var_names:
                ...
                plt.savefig(
                    f"figures/spatial/svg/{lib_id}_{gene}_spatial.pdf", bbox_inches="tight", dpi=300
                )
                plt.savefig(
                    f"figures/spatial/svg/{lib_id}_{gene}_spatial.png", bbox_inches="tight", dpi=300
                )
                plt.close()
    ...
    if len(top_10_svgs) > 0:
        ...
        plt.suptitle(
            "Top 10 Spatially Variable Genes: Non-Solid vs Solid", fontsize=12, fontweight="bold"
        )
        plt.tight_layout()
...
print("=" * 70)
print("   - SVG results: results/svg/morans_i_results.csv")
print("   - SVG plots: figures/spatial/svg/")
if os.path.exists("results/ligrec/ligrec_pvalues.csv"):
    print("   - Ligand-receptor results: results/ligrec/ligrec_pvalues.csv")
    print("   - Ligand-receptor plots: figures/spatial/ligrec/")
