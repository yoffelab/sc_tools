...
def _validate_inputs(adata, cluster_key: str, library_id_key: str) -> None:
    ...
    assert pd.api.types.is_categorical_dtype(adata.obs[cluster_key]), (
        f"obs['{cluster_key}'] must be categorical"
    )
    ...
    assert n_cats == n_cols, (
        f"length mismatch between {cluster_key} categories ({n_cats}) and colors ({n_cols})"
    )


def _build_ordered_samples(
    library_ids: dict[str, list[str]], present_samples: Sequence[str]
...
def _subset_by_compartment(
    df_counts: pd.DataFrame, labels: Sequence[str], wanted: str
) -> pd.DataFrame:
...
def _legend_from_colors(
    ax, labels: Sequence[str], color_map: dict[str, str], ncols: int = 4
) -> None:
    handles = [
...
def plot_five_row_stacked(
    ...
    if len(ordered_samples) == 0:
        raise ValueError(
            "No matching library_id values from the grouping dictionary were found in the AnnData object"
        )
    ...
    if props_epi.shape[1] > 0:
        _draw_stacked(
            ...
            color_map,
            show_percent=True,
            title="Epithelial proportion (within epithelial)",
            bar_width=bar_width,
        )
    else:
    ...
    if props_str.shape[1] > 0:
        _draw_stacked(
            ...
            color_map,
            show_percent=True,
            title="Stromal proportion (within stromal)",
            bar_width=bar_width,
        )
    else:
    ...
    if props_imm.shape[1] > 0:
        _draw_stacked(
            ...
            color_map,
            show_percent=True,
            title="Immune proportion (within immune)",
            bar_width=bar_width,
        )
    else:
    ...
    for grp, xmid in group_centers:
        ax_imm.text(
            ...
            ha="center",
            va="top",
            transform=ax_imm.get_xaxis_transform(),
            fontsize=10,
        )
...
plot_five_row_stacked(
    adata, cluster_key=cluster_key, library_id_key=library_id_key, library_ids=library_ids
)
