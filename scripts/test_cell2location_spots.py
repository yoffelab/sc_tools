...
log_file = os.path.join(
    log_dir, f"cell2location_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
)
logging.basicConfig(
    level=logging.INFO,
...
def get_cache_key(
    sc_data_file: str, celltype_key: str, sc_batch_key: str, n_genes_max: int, skip_hvg: bool
) -> str:
...
def load_single_cell_data(
    sc_data_file: str, celltype_key: str, sc_batch_key: str, qc_labels: list
) -> tuple:
...
def test_cell2location(
    ...
    try:
        ...
        try:
            mod.train(
                max_epochs=actual_epochs, use_gpu=use_gpu, progress_bar_refresh_rate=10
        ...
        c2l.models.Cell2location.setup_anndata(spatial_subset_sig, batch_key=None)
        ...
        if "q05_cell_abundance_w_sf" in spatial_subset_sig.obsm:
            ...
            raise ValueError(
                f"Could not find Cell2location proportions in obsm. Available keys: {list(spatial_subset_sig.obsm.keys())}"
            )
...
def main():
    ...
    spatial_lib = spatial_adata[
        spatial_adata.obs[CONFIG["spatial_batch_key"]] == test_library
    ].copy()
    ...
    for n_spots in CONFIG["test_spot_counts"]:
        ...
        if n_spots > len(spatial_lib):
            logger.warning(
                f"Requested {n_spots} spots but only {len(spatial_lib)} available. Using all spots."
            )
    ...
    print(
        summary_df[
            ["n_spots", "success", "error_type", "proportions_shape", "memory_final_mb"]
        ].to_string()
    )
