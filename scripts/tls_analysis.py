...
adata.obs["solidity_type"] = pd.Categorical(
    adata.obs["solidity_type"], categories=["Normal", "Non-Solid", "Solid"]
)
adata_tls.obs["solidity_type_majority"] = pd.Categorical(
    adata_tls.obs["solidity_type_majority"], categories=["Normal", "Non-Solid", "Solid"]
)
...
adata_tls.obs["B_Naive_ratio"] = np.where(
    b_total > 0, adata_tls.obs["B_Naive_score"] / b_total, np.nan
)
adata_tls.obs["B_Memory_ratio"] = np.where(
    b_total > 0, adata_tls.obs["B_Memory_score"] / b_total, np.nan
)
...
adata_tls.obs["T_Exhausted_ratio"] = np.where(
    t_total > 0, adata_tls.obs["T_Exhausted_score"] / t_total, np.nan
)
adata_tls.obs["T_Effector_ratio"] = np.where(
    t_total > 0, adata_tls.obs["T_Effector_score"] / t_total, np.nan
)
...
def plot_ratio_boxplot(adata, ratio_col, title, stats_df, output_path, figsize=(6, 4)):
    ...
    plot_data = [
        adata.obs.loc[adata.obs["solidity_type_majority"] == g, ratio_col].dropna().values
        for g in groups
    ]
...
plot_ratio_boxplot(
    ...
    "B-cell: Naive Ratio",
    all_stats,
    "figures/tls_spatial/cell_states/B_Naive_ratio_boxplot.pdf",
)
plot_ratio_boxplot(
    adata_tls,
    "B_Memory_ratio",
    "B-cell: Memory Ratio",
    all_stats,
    "figures/tls_spatial/cell_states/B_Memory_ratio_boxplot.pdf",
)
...
plot_ratio_boxplot(
    adata_tls,
    "T_Exhausted_ratio",
    "T-cell: Exhausted Ratio",
    all_stats,
    "figures/tls_spatial/cell_states/T_Exhausted_ratio_boxplot.pdf",
)
plot_ratio_boxplot(
    adata_tls,
    "T_Effector_ratio",
    "T-cell: Effector Ratio",
    all_stats,
    "figures/tls_spatial/cell_states/T_Effector_ratio_boxplot.pdf",
)
...
adata.obs["region_type"] = pd.Categorical(
    adata.obs["region_type"], categories=["TLS", "Tumor", "Other"]
)
...
print("=" * 70)
print("   - Cell state statistics: results/tls/cell_state_ratios_stats.csv")
print("   - Cell state plots: figures/tls_spatial/cell_states/")
print("   - Ligand-receptor results: results/tls/ligrec/")
print("   - Ligand-receptor plots: figures/tls_spatial/ligrec/")
