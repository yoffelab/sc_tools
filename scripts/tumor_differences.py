...
print(f"Testing {len(available_programs)} programs across {len(adata.obs['tumor_type'].cat.categories)} tumor types")
...
def plot_program_boxplot(adata, program_col, group_col='tumor_type', stats_df=None, 
                         output_path=None, figsize=(6, 4)):
    """
    ...
    if stats_df is not None:
        ...
        if len(valid_comparisons) > 0:
            ...
            for idx, comp in enumerate(sig_comparisons):
                ...
                ax.text(x_asterisk, y_bar + bar_height * 0.4, comp['asterisk'],
        ...
        if len(comparison_texts) > 0:
            ...
            fig.text(0.5, 0.01, comparison_str, 
...
for program in tumor_programs:
    if program in available_programs:
        ...
        plot_program_boxplot(adata, program, group_col='tumor_type', 
                           stats_df=stats_combined, output_path=output_pdf)
        plot_program_boxplot(adata, program, group_col='tumor_type', 
                           stats_df=stats_combined, output_path=output_png)
...
print(f"   - Statistics saved to: figures/stats/tumor_differences_1vsall_stats.csv")
print(f"   - Plots saved to: figures/manuscript/tumor_differences/")
